...
class Camera:
    ...
    def trackBall(self, center):
        ...
        self.line_v = [[frame.shape[1]/2, frame.shape[0]*1/2],
                       [frame.shape[1]/2, frame.shape[0]]]
        self.line_v_left = [[frame.shape[1]*2/5, frame.shape[0]
                             * 1/3], [frame.shape[1]*2/5, frame.shape[0]*1/2]]
        self.line_v_right = [[frame.shape[1]*3/5, frame.shape[0]
                              * 1/3], [frame.shape[1]*3/5, frame.shape[0]*1/2]]
        self.line_h = [[frame.shape[1]*1/10, frame.shape[0]*3/4],
                       [frame.shape[1]*9/10, frame.shape[0]*3/4]]
        self.line_h_top = [[frame.shape[1]*2/5, frame.shape[0]
                            * 1/2], [frame.shape[1]*3/5, frame.shape[0]*1/2]]

        color = (0, 0, 255)
        thickness = 3

        self.draw_line(frame, (int(self.line_v[1][0]), int(self.line_v[1][1])), (int(
            self.line_v[0][0]), int(self.line_v[0][1])), color, thickness)
        self.draw_line(frame, (int(self.line_h[1][0]), int(self.line_h[1][1])), (int(
            self.line_h[0][0]), int(self.line_h[0][1])), color, thickness)
        self.draw_line(frame, (int(self.line_h_top[1][0]), int(self.line_h_top[1][1])), (int(
            self.line_h_top[0][0]), int(self.line_h_top[0][1])), color, thickness)
        self.draw_line(frame, (int(self.line_v_left[1][0]), int(self.line_v_left[1][1])), (int(
            self.line_v_left[0][0]), int(self.line_v_left[0][1])), color, thickness)
        self.draw_line(frame, (int(self.line_v_right[1][0]), int(self.line_v_right[1][1])), (int(
            self.line_v_right[0][0]), int(self.line_v_right[0][1])), color, thickness)
